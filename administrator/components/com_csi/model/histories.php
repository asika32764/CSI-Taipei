 ...
 * @since 1.0
 */
class CsiModelHistories extends ListModel
{
	/**
	 * Component prefix.
	 *
	...
	{
		$queryHelper = $this->getContainer()->get('model.histories.helper.query', Container::FORCE_NEW);

		$queryHelper->addTable('history', '#__csi_histories')
			->addTable('user', '#__users', 'history.user_id = user.id')
			->addTable('page', '#__csi_pages', 'page.id = history.page_id')
			->addTable('task', '#__csi_tasks', 'task.id = history.task_id')
